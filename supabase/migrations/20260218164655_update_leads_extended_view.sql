DROP VIEW IF EXISTS public.leads_extended;
...
SELECT
  ...
  -- ICP Columns
  icp.title AS icp_title,
  icp.tag_color as icp_tag_color
FROM leads l
LEFT JOIN prospects p ON l.prospect_id = p.id
LEFT JOIN intended_customer_profiles icp on p.tagged_icp_id = icp.id
...
CREATE INDEX IF NOT EXISTS idx_prospects_tagged_icp ON prospects(tagged_icp_id);