...
CREATE OR REPLACE FUNCTION public.debug_team_filter()
 RETURNS TABLE(rep_id uuid, rep_name text, rep_department text, my_department text, my_role text, dept_match_raw boolean, dept_match_trimmed boolean, should_include boolean)
 LANGUAGE plpgsql
 SECURITY DEFINER
AS $function$
DECLARE
  v_my_department TEXT := (SELECT department FROM profiles WHERE id = auth.uid() LIMIT 1);
  v_my_role TEXT := (SELECT role FROM profiles WHERE id = auth.uid() LIMIT 1);
BEGIN
  RETURN QUERY
  SELECT 
    p.id,
    ...
    v_my_department,
    v_my_role,
    (p.department::text = v_my_department::text) AS dept_match_raw,
    (TRIM(LOWER(p.department::text)) = TRIM(LOWER(v_my_department::text))) AS dept_match_trimmed,
    (
      v_my_role = 'admin'
      OR (
        p.department IS NOT NULL 
        AND v_my_department IS NOT NULL
        AND TRIM(LOWER(p.department::text)) = TRIM(LOWER(v_my_department::text))
      )
    ) AS should_include
  FROM profiles p
...
DECLARE
  ...
  team_deals JSON;
  team_response JSON;
  personal_response JSON;

  -- ✅ Capture user context ONCE
  v_user_id UUID := auth.uid();
  v_user_role TEXT := (SELECT role FROM profiles WHERE id = auth.uid() LIMIT 1);
  v_user_department TEXT := (SELECT department FROM profiles WHERE id = auth.uid() LIMIT 1);
BEGIN
  ...
  -- 8. Team Performance: Leads (✅ FIXED: Added department filter with variables)
  SELECT json_agg(json_build_object(
    'rep_id', p.id,
  ...
  AND (
    ...
    OR (
      ...
      AND TRIM(LOWER(p.department::text)) = TRIM(LOWER(v_user_department::text))
    )
  );

  -- 9. Team Performance: Deals (✅ FIXED: Added department filter with variables)
  SELECT json_agg(json_build_object(
    'rep_id', p.id,
    'rep_name', p.full_name,
  ...
  -- 10. Team Performance: Response Time (✅ FIXED: Use v_user_department NOT department())
  SELECT json_agg(json_build_object(
    'rep_id', p.id,
    'rep_name', p.full_name,
  ...
  RETURN json_build_object(
    ...
    'personal_revenue_by_stream', coalesce(personal_revenue_by_stream, '[]'::json),
    'won_deals_by_industry', coalesce(won_deals_by_industry, '[]'::json),
    'personal_won_deals_by_industry', coalesce(personal_won_deals_by_industry, '[]'::json),
    'leads_by_industry', coalesce(leads_by_industry, '[]'::json),
    'personal_leads_by_industry', coalesce(personal_leads_by_industry, '[]'::json),
    'team_leads', coalesce(team_leads, '[]'::json),
    'team_deals', coalesce(team_deals, '[]'::json),
    'team_response', coalesce(team_response, '[]'::json),
    'personal_response', coalesce(personal_response, '[]'::json)
  );
END;
$function$
;
...
BEGIN
  RETURN json_build_object(
    ...
    'role_type', pg_typeof(role()),
    'dept_type', pg_typeof(department())
  );
END;
$function$
;
